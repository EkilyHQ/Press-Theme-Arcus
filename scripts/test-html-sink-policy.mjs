...
assert.deepEqual(
  ...
  'bound Object and Reflect helper aliases must retain both indirect-helper and concrete reflected-write findings'
);
...
for (const filePath of ['theme/modules/interactions.js', 'theme/modules/layout.js']) {
  const arcusSource = await readFile(path.join(SCRIPT_DIR, '..', filePath), 'utf8');
  ...
  assert.deepEqual(
    ...
    withoutLocations(
      scanJavaScriptSource({
        filePath,
        source: formattedArcusSource,
        wrapperNames: policy.wrappers.map(({ name }) => name)
      })
    ),
    `the exact Arcus sink inventory must remain stable when Prettier formats ${filePath}`
  );
}
