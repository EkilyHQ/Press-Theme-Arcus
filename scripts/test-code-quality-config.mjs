...
const EXPECTED_DISABLED_RULES = ['no-empty', 'no-unused-vars', 'no-useless-assignment'];
...
assert.match(eslintConfig, /legacy inert ESLint directive moved outside its reviewed context/u);
assert.match(eslintConfig, /'theme\/modules\/interactions\.js'/u);
assert.match(eslintConfig, /processor: legacyInertDirectiveProcessor/u);
...
assert.equal(policy.scope, 'Arcus first-party theme source and repository tooling');
...
assert.deepEqual(policy.eslint?.inlineConfiguration, {
  ...
  policy:
    "Source comments cannot disable or reconfigure ESLint rules. The executable proof loads the real project configuration and confirms a used no-undef directive neither suppresses nor downgrades the severity-2 diagnostic. Arcus's one historical no-restricted-globals comment is masked only to avoid noInlineConfig's inert-comment warning; it cannot alter rule results and duplicate occurrences fail lint."
});
assert.deepEqual(policy.eslint?.legacyInertDirective, {
  path: 'theme/modules/interactions.js',
  source: '// eslint-disable-line no-restricted-globals',
  reviewedContext: 'location.href = href; // eslint-disable-line no-restricted-globals',
  observedOccurrences: 1,
  decision: 'accepted-inert-comment',
  evidence:
    'The comment predates this repository configuration, targets a rule that is not part of the project policy, and is rendered inert by noInlineConfig. A narrowly scoped processor masks only its warning text so the zero-warning lint gate remains exact without changing release-bearing theme source.'
});
...
for (const token of [
  ...
  assert.ok(eslintDebtProbe.includes(token), `exact ESLint debt probe must retain ${token}`);
}
...
assert.deepEqual(eslintDebtBaseline.rules, EXPECTED_DISABLED_RULES);
assert.deepEqual(eslintDebtBaseline.expectedCounts, {
  'no-empty': 42,
  'no-unused-vars': 71,
  'no-useless-assignment': 1
});
assert.equal(eslintDebtBaseline.diagnostics.length, 114);
...
assert.equal(new Set(eslintDebtKeys).size, eslintDebtKeys.length, 'exact ESLint debt identities must be unique');
for (const diagnostic of eslintDebtBaseline.diagnostics) {
  assert.match(diagnostic.path, /^theme\/.*\.(?:js|mjs)$/u);
  assert.ok(diagnostic.owner.length > 0);
  assert.ok(diagnostic.ownerPath.length > 0);
  assert.match(diagnostic.fingerprint, /^sha256:[0-9a-f]{64}$/u);
  assert.equal('line' in diagnostic, false, 'line movement must be represented by the stable context fingerprint');
  assert.equal('column' in diagnostic, false, 'column movement must be represented by the stable context fingerprint');
}
assert.equal(policy.prettier?.baseline?.file, 'scripts/prettier-baseline.json');
assert.equal(policy.prettier?.baseline?.initialFiles, 11);
...
assert.equal(prettierBaseline.files.length, 11);
...
assert.deepEqual(htmlPolicy.expectedKinds, {
  'html-wrapper-call:applyOptions': 5,
  'html-wrapper-call:clearArcusToc': 6,
  'html-wrapper-call:renderFooterLinks': 1,
  'html-wrapper-call:renderLinksList': 1,
  'html-wrapper-call:renderLoader': 2,
  'html-wrapper-call:renderNavLinks': 1,
  'html-wrapper-call:renderStaticView': 1,
  'html-wrapper-call:resetToolsPanel': 1,
  'html-wrapper-call:setupToolsPanel': 3,
  'html-wrapper-call:showToc': 3,
  'innerHTML-write': 27
});
assert.equal(htmlPolicy.approved.length, 51, 'all direct writes and reviewed wrapper calls must remain classified');
