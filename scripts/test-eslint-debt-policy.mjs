...
const arcusSource = await readFile(new URL('../theme/modules/interactions.js', import.meta.url), 'utf8');
const formattedArcusSource = await prettier.format(arcusSource, {
  parser: 'babel',
  printWidth: 120,
  semi: true,
  singleQuote: true,
...
assert.deepEqual(
  createBaseline(scanJavaScriptSource({ filePath: 'theme/modules/interactions.js', source: arcusSource })),
  createBaseline(scanJavaScriptSource({ filePath: 'theme/modules/interactions.js', source: formattedArcusSource })),
  'the exact Arcus debt baseline must remain stable when the touched-legacy Prettier rule formats its theme file'
);
